...
The **C implementation** uses ```Nettle``` (version 3.6) and any C library - it should work with ```glibc``` and ```uClibc``` without problems. If any changes are needed to use ```dietlibc```, your pull requests are very welcome.
...
If you want to build on MacosX with Nettle, you need to specify ```DECODER_CONFIG_LINK_STATIC=n```
